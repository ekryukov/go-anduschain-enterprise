...
func TestDAOForkRangeExtradata(t *testing.T) {
	...
	proBc, _ := NewBlockChain(proDb, nil, &proConf, deb.NewFaker(), vm.Config{})
	defer proBc.Stop()
	...
	conBc, _ := NewBlockChain(conDb, nil, &conConf, deb.NewFaker(), vm.Config{})
	defer conBc.Stop()
	...
	for i := int64(0); i < params.DAOForkExtraRange.Int64(); i++ {
		...
		db = ethdb.NewMemDatabase()
		gspec.MustCommit(db)
		bc, _ := NewBlockChain(db, nil, &conConf, deb.NewFaker(), vm.Config{})
		defer bc.Stop()
		...
		db = ethdb.NewMemDatabase()
		gspec.MustCommit(db)
		bc, _ = NewBlockChain(db, nil, &proConf, deb.NewFaker(), vm.Config{})
		defer bc.Stop()
